...
SELECT 
    calendar_year
    ,CAST(ROUND(100.0 * MAX (CASE WHEN demographic = 'families' THEN yearly_sales ELSE NULL END)
        / SUM(yearly_sales) ,2 ) AS DECIMAL(10,2)) AS sales_families
    ,CAST(ROUND(100.0 * MAX (CASE WHEN demographic = 'couples' THEN yearly_sales ELSE NULL END)
        / SUM(yearly_sales) ,2 ) AS DECIMAL(10,2)) AS sales_couples
    ,CAST(ROUND(100.0 * MAX (CASE WHEN demographic = 'unknown' THEN yearly_sales ELSE NULL END) 
        /SUM(yearly_sales),2 ) AS DECIMAL(10,2)) AS sales_unknown
FROM sales_by_demo
GROUP BY calendar_year
...
- demographic
- customer_type
Do you have any further recommendations for Danny’s team at Data Mart or any interesting insights based 
off this analysis?*/
SELECT 
    metric
    ,value
    ...
    ,sales_after
    ,sales_after - sales_before AS sales_diff
    ,CAST(ROUND (100.0 * (sales_after - sales_before) / NULLIF(sales_before, 0), 2) AS DECIMAL (10,2)) AS percent_diff
FROM (
    --REGION 
    SELECT 
        'region' AS metric
        ,COALESCE(region, 'unknown') AS value
        ,SUM(CAST(CASE WHEN calendar_year = 2020 AND week_number BETWEEN 13 AND 24 THEN sales END AS BIGINT)) AS sales_before
        ,SUM(CAST(CASE WHEN calendar_year = 2020 AND week_number BETWEEN 25 AND 37 THEN sales END AS BIGINT)) AS sales_after 
    FROM clean_weekly_sales
    GROUP BY region
    ...
    SELECT 
        'platform' AS metric
        ,COALESCE(platform, 'unknown') AS value
        ,SUM(CAST(CASE WHEN calendar_year = 2020 AND week_number BETWEEN 13 AND 24 THEN sales END AS BIGINT)) AS sales_before
        ,SUM(CAST(CASE WHEN calendar_year = 2020 AND week_number BETWEEN 25 AND 37 THEN sales END AS BIGINT)) AS sales_after 
    FROM clean_weekly_sales
    GROUP BY platform    
    ...
    SELECT 
        'age_band' AS metric
        ,COALESCE(age_band, 'unknown') AS value
        ,SUM(CAST(CASE WHEN calendar_year = 2020 AND week_number BETWEEN 13 AND 24 THEN sales END AS BIGINT)) AS sales_before
        ,SUM(CAST(CASE WHEN calendar_year = 2020 AND week_number BETWEEN 25 AND 37 THEN sales END AS BIGINT)) AS sales_after 
    FROM clean_weekly_sales
    GROUP BY age_band   
    ...
    SELECT 
        ...
        ,COALESCE(demographic, 'unknown') AS value
        ,SUM(CAST(CASE WHEN calendar_year = 2020 AND week_number BETWEEN 13 AND 24 THEN sales END AS BIGINT)) AS sales_before
        ,SUM(CAST(CASE WHEN calendar_year = 2020 AND week_number BETWEEN 25 AND 37 THEN sales END AS BIGINT)) AS sales_after 
    FROM clean_weekly_sales
    ...
    SELECT 
        ...
        ,COALESCE(customer_type, 'unknown') AS value
        ,SUM(CAST(CASE WHEN calendar_year = 2020 AND week_number BETWEEN 13 AND 24 THEN sales END AS BIGINT)) AS sales_before
        ,SUM(CAST(CASE WHEN calendar_year = 2020 AND week_number BETWEEN 25 AND 37 THEN sales END AS BIGINT)) AS sales_after 
    FROM clean_weekly_sales
...
WITH metric_changes AS(
    SELECT 
        ...
        ,SUM(CAST(CASE WHEN calendar_year = 2020 AND week_number BETWEEN 25 AND 37 THEN sales END AS BIGINT)) AS sales_after
    FROM (
        SELECT 'region' AS metric ,region AS value , * FROM clean_weekly_sales  
        UNION ALL
        SELECT 'platform' AS metric ,platform AS value , * FROM clean_weekly_sales
        UNION ALL
        SELECT 'age_band' AS metric ,age_band AS value , * FROM clean_weekly_sales
        UNION ALL
        SELECT 'demographic' AS metric ,demographic AS value , * FROM clean_weekly_sales
        UNION ALL
        SELECT 'customer_type' AS metric ,customer_type AS value , * FROM clean_weekly_sales
    ) AS unified
...
SELECT 
    ...
    ,sales_after - sales_before AS sales_diff
    ,CAST(ROUND (100.0 * (sales_after - sales_before) / NULLIF(sales_before, 0), 2) AS DECIMAL (10,2)) AS percent_diff
    ,ROW_NUMBER() OVER (PARTITION BY metric ORDER BY CAST(ROUND (100.0 * (sales_after - sales_before) / NULLIF(sales_before, 0), 2) AS DECIMAL (10,2))) AS rn
FROM metric_changes
)
SELECT 
    metric
